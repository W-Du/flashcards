...
from forms import AddVocabularyForm, AddListForm, ChangeDailyGoalForm, WordPriorityForm
...
def practice(username, cur_word_idx = 0):
    ...
    if username != user.username:
        return redirect(url_for('login.login'))
    if user.daily_goal:
        if len(list(user.words)) >= user.daily_goal:
            words_today = Word.query.order_by(Word.priority.desc(), func.random()).limit(user.daily_goal)
        else:
            words_today = Word.query.order_by(Word.priority, fun.random()).all()
    else:
        words_today = Word.query.all()
    ...
    return render_template('practice.html', user=user, cur_word_idx=cur_word_idx, cur_word=cur_word, length=length)
...
def word_priority(username):
    ...
    try:
        ...
        cur_index = request.form.get('index')
        ...
        print(str(e))
    return redirect(url_for('user.practice', username=username, index=int(cur_index)+1))
    ...
    if form_id == 'list':
        if formList.validate_on_submit():
            ...
            if new_list:
                ...
                new_list.users.append(user)
                user.lists.append(new_list)
                db.session.commit()
                formList.listname.data = None
                return render_template('setting.html', user = user, formList = formList, formGoal = formGoal)
            else:
...
@user_bp.route('/user/<username>/setting/lists')
@login_required
...
def getAllWords(username):
    ...
    return render_template('allWords.html', user=user, words=words)
